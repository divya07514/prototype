...
// We try to pick first available seat here. Thread contention will be there and same seat can be booked by multiple users.
func main() {
	DB, err := dbConn()
	service.Init()
	if err != nil {
...
func book(db *sql.DB, user *service.User) (*service.Seat, error) {
	...
	row := txn.QueryRow("select id, name, trip_id, user_id from seats where user_id is null order by id Limit 1")
	if row.Err() != nil {
		return nil, row.Err()
	}
